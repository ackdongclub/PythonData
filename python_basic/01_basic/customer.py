...
# - 괄호 안 영문자를 입력하면(대소문자 구분 없이) 각 기능이 구현되게 만든다
...
while True:
    ...
    if choice=="I":  
        ...
        while True:
            ...
            while True:
                ...
                if result:
                    ...
                    print('@를 포함하여 정확한 이메일 주소를 입력해주세요')
            check = 0
            for i in custlist:
        ...
        while True:
            customer['birth'] = input('출생년도 4자리를 입력하세요 >>>')
        ...
        if page < 0:
            ...
            print(f'현재 페이지는 {page + 1}째 페이지입니다')
            print(custlist[page])
        ...
        if page <= 0:
            print(f'첫번째 페이지입니다. 이전페이지 이동 불가입니다')
        else:
            page -= 1
            print(f'현재 페이지는 {page + 1}째 페이지입니다')
            print(custlist[page])
        ...
        while True:
            ...
            if idx == -1:
                ...
                break
            choice1 = input('''
다음 중 수정할 항복을 선택하세요(name, gender, birth)
>>> ''')
