...
class Customer:
    ...
    def customer_input(self):
        ...
        self.page = len(customer) - 1
        print(customer)
        print(self.custlist)
        return self.page # 데이터 값이 복사되어 넘어올 경우 변경 값을 꼭 리턴시켜줘야 한다.

    def cur_page(self):
        if self.page < 0:
            print('입력된 정보가 없습니다.')
        else:
            print(f'현재 페이지는 {self.self.page+1}페이지 입니다.')
    ...
    def pre_page(self):
        if self.page <= 0:
            ...
            print(self.custlist[self.page])
        print(self.page)
        return self.page #페이지 값을 -1씩 변경 되기에 리턴해줘야 한다.
    ...
    def exe(self):
        ...
        if choice == "I":  
            self.customer_input() # self.page로 변경된 리턴값 받아줘야 한다.
        elif choice =="C": 
            self.cur_page()
        elif choice == 'P':
            self.pre_page()
